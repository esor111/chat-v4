...
export class EnhancedMessageRepository {
  ...
  async getConversationHistory(
    ...
    try {
      ...
      if (beforeMessageId) {
        query = query.andWhere("message.id < :beforeMessageId", {
          beforeMessageId,
        });
      }
      ...
      const messagesWithSender: MessageWithSender[] = messages
        .reverse()
        .map((message) => ({
          ...message,
  ...
  async getMessageById(messageId: string): Promise<MessageWithSender | null> {
    try {
  ...
  async getMessagesForRetention(): Promise<MessageRetentionStats> {
    try {
      ...
      const softDeleteCutoff = new Date(
        now.getTime() - this.SOFT_DELETE_DAYS * 24 * 60 * 60 * 1000
      );
      const hardDeleteCutoff = new Date(
        now.getTime() - this.HARD_DELETE_DAYS * 24 * 60 * 60 * 1000
      );
  ...
  async applySoftDeleteRetentionPolicy(
  ...
  async applyHardDeleteRetentionPolicy(
    batchSize: number = 1000
  ): Promise<number> {
    try {
  ...
  async getConversationMessageStats(conversationId: string): Promise<{
    totalMessages: number;
    deletedMessages: number;
    messagesByType: Record<string, number>;
  }> {
    try {
      const [totalMessages, deletedMessages, messagesByTypeResult] =
        await Promise.all([
          this.messageRepository.count({
            where: { conversationId, deletedAt: IsNull() },
          }),
          this.messageRepository.count({
            where: { conversationId, deletedAt: Not(IsNull()) },
          }),
